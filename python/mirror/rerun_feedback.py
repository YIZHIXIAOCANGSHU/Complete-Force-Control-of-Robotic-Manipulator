...
class UsbfdcanSimRerunRecorder:
    def __init__(self, *, motor_ids: tuple[int, ...] = (1, 2, 3, 4, 5, 6, 7), spawn: bool = True) -> None:
        ...
        if not self._enabled:
            return
        rr.init("AM-D02 CAN 反馈镜像仿真", spawn=spawn)
        self._setup_styles()
    ...
    def _setup_styles(self) -> None:
        ...
        for name in ("tx_send_rate_hz", "rx_feedback_rate_hz", "complete_round_rate_hz", "missing_feedback_mask", "backpressure_count"):
            rr.log(f"mirror/performance/{name}", rr.SeriesLines(names=[name], widths=[2]), static=True)

    def _setup_blueprint(self) -> None:
        if not self._enabled or rrb is None:
            return
        performance_base = "/mirror/performance"
    ...
    def _motor_path(motor_id: int) -> str:
        return f"mirror/motors/motor_{int(motor_id):02d}"
    ...
    def log_performance(
        ...
        *,
        ...
        if raw_zero_packet:
            rr.log("mirror/raw_zero_packet", rr.TextDocument(raw_zero_packet.hex(" ").upper()))
